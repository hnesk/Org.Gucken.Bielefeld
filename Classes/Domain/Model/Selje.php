...
class Selje extends AbstractEventSource implements EventSourceInterface {
	...
	public function getEvents() {
		$url = $this->getUrl();
		$urls = \newdate()->getMonthRange(2)->map(function(Date $d) use ($url) { 
	...
	public function getEvent(\Type\Xml $xml) {	
		...
		$dates = $xml->xpath('./*')->asString()->normalizeSpace()
			->expandRanges('#(?<from>\d+)\.\s*(?<op>[+-])\s*(?<to>\d+)\.\s*(\d+)\.\s+(\d+)[.:](\d+)#', '_item._4. _5:_6', ', ')
			->eachMatch('#(\d+)\.(\d+)\.\s+(\d+).(\d+)#')
			->asDate('%d.%m.\s+%H.%M');
		...
		return $dates->map(function (\Type\Date $date) use($url, $title, $detailUrl, $title, $description, $image, $type, $location) {
			$data = array(
				'url' => $url,
				'date' => $date,
				'title' => $title,
